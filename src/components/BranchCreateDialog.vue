  ...
  <div v-if="visible && dialog"
    ...
    <div class="rounded-xl w-[480px] flex flex-col overflow-hidden"
      ...
      <div class="flex items-center gap-2 px-4 h-12 shrink-0" style="border-bottom: 1px solid var(--border-soft)">
        ...
        <span class="font-semibold text-[13px]">New Branch</span>
        <span class="text-[11px]" style="color: var(--fg-3)">from</span>
      ...
      <div class="p-3">
        <input ref="input" v-model="name"
               class="gl-input gl-mono text-[12.5px] w-full"
               placeholder="Branch name…" />
      </div>

      <div class="flex items-center gap-2 px-4 py-3 shrink-0" style="border-top: 1px solid var(--border-soft)">
        <span class="text-[11px]" style="color: var(--fg-3)">↩ Create · Esc Cancel</span>
        <div class="flex-1" />
        <button class="gl-btn gl-btn-ghost" :disabled="submitting" @click="cancel">Cancel</button>
        <button class="gl-btn gl-btn-primary" :disabled="submitting || !name.trim()" @click="submit">
