...
function statusMeta(s: ChangedFile["status"]) {
  switch (s) {
    case "added":      return { letter: "A", color: "var(--status-added)",      bg: "var(--status-added-bg)" };
    case "modified":   return { letter: "M", color: "var(--status-modified)",   bg: "var(--status-modified-bg)" };
    case "deleted":    return { letter: "D", color: "var(--status-deleted)",    bg: "var(--status-deleted-bg)" };
    case "renamed":    return { letter: "R", color: "var(--status-renamed)",    bg: "var(--status-renamed-bg)" };
    case "copied":     return { letter: "C", color: "var(--status-copied)",     bg: "var(--status-copied-bg)" };
    case "typechange": return { letter: "T", color: "var(--status-typechange)", bg: "var(--status-typechange-bg)" };
  }
}
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div class="p-3 flex flex-col gap-1.5 shrink-0 gl-selectable"
      ...
      <div class="text-[13.5px] font-semibold gl-selectable" style="color: var(--fg)">{{ detail.subject }}</div>
      <pre v-if="detail.body.trim()"
           class="text-[13px] whitespace-pre-wrap gl-selectable gl-mono m-0"
           style="color: var(--fg-2)">{{ detail.body.trim() }}</pre>
      <div class="flex items-center gap-1.5 flex-wrap text-[12px]" style="color: var(--fg-2)">
      ...
      <div v-if="detail.refs.length" class="flex items-center gap-1 flex-wrap text-[12px]">
      ...
      <div v-if="detail.parents.length" class="text-[12px] flex gap-1.5 flex-wrap" style="color: var(--fg-3)">
        Parents:
